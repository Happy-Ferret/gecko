...
var fetcher = module.exports = function(os) {
  // Ensure that the top level directory into which we'll install xulrunner exists.
  Moztarball.prototype._checkBuildDir(os.buildRoot);
  this._os = os;
  this._configBlob = os.manifest['mozfetcher']
  this._buildRoot = os.buildRoot;
  this._platform = os.platform;
  this._arch = os.arch;
  this._config = this._getConfig();
  // Maybe we want to move this cache dir into a dot directory in the users
  // system?  would this be horrible, or helpful?
  this._cacheDir = Path.join(this._buildRoot, "cache");
};
...
this.getXulrunnerPath = function() {
  return Path.join(this.getUnpackDir(), this._config.bin.path);
};
...
this.getUnpackDir = function() {
  return Path.join(this._buildRoot, this._platform + '_' + this._arch);
}
...
this.fetchIfNeeded = function(callback) {
  ...
  self.needsFetch(function(err, needsFetch) {
    ...
    if (!needsFetch)
      return callback();
    Mkdirp(self.getUnpackDir(), function(err) {
      if (err) {
        return callback(err);
      }
      new Moztarball(self.getUnpackDir()).unpack(self.getArchivePath(), function(err) {
        if (err) {
          return callback(err);
        }
